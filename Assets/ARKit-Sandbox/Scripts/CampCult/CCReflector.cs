...
public class CCReflector<T>{
	public T type;
	public string[] types =new string[]{ typeof(float).ToString(), typeof(System.Int32).ToString(), typeof(Vector2).ToString(),typeof(Vector3).ToString(),typeof(Vector4).ToString()};
	public Object obj;
    ...
    public void Draw(Rect r)
    {
        ...
        if (o == null)
            o = (Object)this.obj;
#if UNITY_EDITOR
        Object objField = (Object)EditorGUI.ObjectField(new Rect(r.x, r.y, r.width * .5f, r.height), o, typeof(Object), true);
#else
        Object objField = this.obj;
#endif
        if (objField != null)
        {
           isMat = false;
            if (obj.GetType() == typeof(Material))
            {
                ...
                this.obj = objField;
                isMat = true;
                GetMatFields((Material)objField, GetArray(types));
                DisplayVarsDropdown( new Rect(r.x + r.width * .5f, r.y, r.width * .5f, r.height));
            }
            else if (objField is GameObject)
            {
                go = (GameObject)objField;
                GetComponents((GameObject)objField);
                ...
                this.obj = ((GameObject)objField).GetComponent(System.Type.GetType(components[componentIndex]));
            ...
            else
            {
                go = null;
                this.obj = objField;
                GetFields(objField.GetType(), GetArray(types));
                DisplayVarsDropdown( new Rect(r.x + r.width * .5f, r.y, r.width * .5f, r.height));
            }
        }
    }
